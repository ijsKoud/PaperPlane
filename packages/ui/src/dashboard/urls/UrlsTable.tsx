...
export const ShortUrlsTable: React.FC<Props> = ({ urls, selected, onSelect }) => {
	return (
		<div className="w-full flex flex-wrap gap-4 items-center justify-center bg-main p-8 rounded-xl">
			<div className="overflow-auto w-full max-w-[calc(100vw-16px-64px)]">
				...
				>
					{urls.map((url, key) => (
						<ShortUrlsTableEntry url={url} key={key} selected={selected.includes(url.name)} onClick={onSelect} />
					))}
				</Table>
			</div>
		</div>
