...
export const DataTable = <TData, TValue>({ columns, data, page, pages, setPage }: DataTableProps<TData, TValue>) => {
	...
	async function deleteInvite() {
		try {
			const invites = table.getFilteredSelectedRowModel().rows;
			await axios.delete("/api/invites/create", { data: { invites: invites.map((url) => url.getValue("invite")) } });
			toast({ title: "Invites Deleted", description: `${invites.length} invites have been deleted.` });
			setRowSelection({});
		} catch (err) {
				...
				<AlertDialog>
					<AlertDialogTrigger asChild>
						<Button className="mt-2" variant="destructive" disabled={table.getFilteredSelectedRowModel().rows.length <= 0}>
							<Trash2Icon className="mr-2 w-4 h-4" /> Delete {table.getFilteredSelectedRowModel().rows.length} invites
						</Button>
					</AlertDialogTrigger>
					...
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								...
								<strong>{table.getFilteredSelectedRowModel().rows.length}</strong> invites from PaperPlane.
							</AlertDialogDescription>
						</AlertDialogHeader>

						<AlertDialogFooter>
							<AlertDialogCancel>Cancel</AlertDialogCancel>
							<AlertDialogAction onClick={deleteInvite}>Delete</AlertDialogAction>
						</AlertDialogFooter>
					</AlertDialogContent>
				</AlertDialog>
