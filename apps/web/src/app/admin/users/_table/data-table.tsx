...
export const DataTable = <TData, TValue>({ columns, data, page, pages, setPage }: DataTableProps<TData, TValue>) => {
	...
	async function deleteUsers() {
		try {
			...
			await axios.delete("/api/admin/create", { data: { domains: domains.map((url) => url.getValue("domain")) } });
			...
			toast({
				...
				action: (
					<ToastAction altText="Try again" onClick={deleteUsers}>
						Try again
					</ToastAction>
				)
			});
				...
				<AlertDialog>
					<AlertDialogTrigger asChild>
						<Button className="mt-2" variant="destructive" disabled={table.getFilteredSelectedRowModel().rows.length <= 0}>
							<Trash2Icon className="mr-2 w-4 h-4" /> Delete {table.getFilteredSelectedRowModel().rows.length} users
						</Button>
					</AlertDialogTrigger>
					...
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								...
								<strong>{table.getFilteredSelectedRowModel().rows.length}</strong> users from PaperPlane.
							</AlertDialogDescription>
						</AlertDialogHeader>

						<AlertDialogFooter>
							<AlertDialogCancel>Cancel</AlertDialogCancel>
							<AlertDialogAction onClick={deleteUsers}>Delete</AlertDialogAction>
						</AlertDialogFooter>
					</AlertDialogContent>
				</AlertDialog>
